...
- **Square is frozen during its platform update (Aug 2026).** Square defaults to frozen: NO Square traffic — no writes AND no Square syncs — until it is explicitly unfrozen. Full syncs skip Square and a Square-only sync is rejected while frozen. Check `bin/rails ops:push_guard:status`. Do not unfreeze without an explicit human instruction.
