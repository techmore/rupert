...
class SettingsController < AuthenticatedController
  ...
  def drive_logs
    rows = BackupLog.recent(10).map do |log|
      {
        status: log.status, fileName: log.fileName, driveUrl: log.driveUrl,
        startedAt: log.startedAt, finishedLabel: log.finishedAt ? log.finishedAt.utc.strftime("%b %e %H:%M UTC") : nil,
        error: log.error
      }
    end
    render json: rows
  ...
  def drive_auth
    ...
    redirect_to GoogleDriveBackupService.auth_url(redirect_uri: drive_oauth_callback_settings_url, state: state),
  ...
  rescue GoogleDriveBackupService::NotConfiguredError, GoogleDriveBackupService::NotConnectedError,
