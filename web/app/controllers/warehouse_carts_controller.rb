...
class WarehouseCartsController < ApplicationController
  ...
  def add_item
    ...
    unless variant
      return redirect_to(warehouse_sale_path(@share.token), alert: "That product is no longer available.")
    end

    available = InventoryLevel.unscoped.where(shopifyVariantId: variant.id).sum(:quantity)
    if quantity > available
      return redirect_to(warehouse_sale_path(@share.token), alert: "Only #{available} of #{variant.title} are available.")
    end
