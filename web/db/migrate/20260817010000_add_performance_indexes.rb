...
class AddPerformanceIndexes < ActiveRecord::Migration[8.1]
  def change
    ...
    add_index :InventoryLevel,
              %i[tenant_id source squareVariationId],
              name: 'idx_inventory_levels_tenant_source_square_variation'
    add_index :InventoryLevel,
              %i[tenant_id source shopifyVariantId],
              name: 'idx_inventory_levels_tenant_source_shopify_variant'
    add_index :LedgerEntry, %i[tenant_id occurredAt], name: 'idx_ledger_entries_tenant_occurred_at'
    add_index :InventoryMovement,
              %i[tenant_id source createdAt],
              name: 'idx_inventory_movements_tenant_source_created_at'
    add_index :SyncRun, %i[status finishedAt], name: 'idx_sync_runs_status_finished_at'
    add_index :ShopifyVariant, %i[sku productId], name: 'idx_shopify_variants_sku_product'

    # Modern snake_case tables.
    add_index :orders, :location_id
    add_index :size_changes, %i[tenant_id status]
  end
end
