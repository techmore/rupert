...
class ScopeUniqueIndexesToTenant < ActiveRecord::Migration[8.1]
  def up
    ...
    add_index :Location, %i[tenant_id source externalId],
    ...
    add_index :InventoryLevel, %i[tenant_id source locationId shopifyVariantId],
              name: 'idx_inventory_levels_tenant_source_loc_shopify', unique: true
    add_index :InventoryLevel, %i[tenant_id source locationId squareVariationId],
  ...
  def down
    ...
    add_index :Location, %i[source externalId], name: 'index_Location_on_source_and_externalId', unique: true
    ...
    add_index :InventoryLevel, %i[source locationId shopifyVariantId],
              name: 'idx_on_source_locationId_shopifyVariantId_9ef5a647f1', unique: true
    add_index :InventoryLevel, %i[source locationId squareVariationId],
