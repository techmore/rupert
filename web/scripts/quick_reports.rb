...
def cmd_sales(args)
  ...
  scope.group(:source).order("1").count.each { |s, n| puts "  #{s}: #{money_cents(scope.where(source: s).sum(:gross_cents))} (#{n} orders)" }
  scope.group(:channel).order("1").count.each { |c, n| puts "  channel #{c}: #{money_cents(scope.where(channel: c).sum(:gross_cents))} (#{n} orders)" }
...
def cmd_reconcile(_args)
  ...
  Reconciler.actionable_rows(rows).sort_by { |r| -r.drift.to_i.abs }.first(5).each do |r|
    puts "  #{r.sku} shopify=#{r.shopify_qty} square=#{r.square_qty} drift=#{r.drift > 0 ? "+" : ""}#{r.drift}  #{r.product[0, 48]}"
  end
end
