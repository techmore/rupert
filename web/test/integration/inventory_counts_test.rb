...
class InventoryCountsTest < ActionDispatch::IntegrationTest
  ...
  test "submit -> approve applies the override" do
    ...
    variant = ShopifyVariant.create!(title: "Widget - Small", sku: "WIDGET-S",
    ...
    InventoryLevel.create!(source: "shopify", locationId: location.id,
      shopifyVariantId: variant.id, quantity: 10, tenant_id: @tenant.id)
  ...
  test "readers cannot create or approve counts" do
    reader = User.create!(email: "reader@example.com", password: "password123",
      role: "reader", tenant: tenants(:default_tenant))
    delete logout_path
