...
class SalesAnnouncerTest < ActiveSupport::TestCase
  ...
  def make_entry(occurred_at, gross:, source: 'square', status: 'COMPLETED', summary: 'Honey Sticks')
    attrs = {
      source: source, sourceOrderId: SecureRandom.hex(8), orderName: 'SQ-test',
      occurredAt: occurred_at, syncedAt: Time.current, currency: 'USD',
      grossCents: gross, status: status, lineItems: 1, summary: summary,
